...
function createHighlights(scene, heightMap, mapVerts, highlights, nodes) {
  ...
  for (let i = 0; i < heightMap.length - 1; i++) {
    for (let j = 0; j < heightMap.length - 1; j++) {
      ...
      let characterSelectPurp = createHighlight(j, i, "#6a0dad"/*purple*/);
      let enemySelectOrg = createHighlight(j, i, "#FFA500"/*orange*/);
      temp.name = "highlightR - " + j + " - " + i;
      temp2.name = "highlightB - " + j + " - " + i;
      characterSelectPurp.name = "highlightP - " + j + " - " + i;
      enemySelectOrg.name = "highlightO - " + j + " - " + i;
      temp.visible = false;
      temp2.visible = false;
      characterSelectPurp.visible = false;
      enemySelectOrg.visible = false;
      nodes[i][j] = createNode(i, j, scene.obstacles);
      highlights[i][j] = temp2;
      scene.add(temp);
      scene.add(temp2);
      scene.add(characterSelectPurp);
      scene.add(enemySelectOrg);
    }
  }
...
function clearEnemyRadius(scene) {
  for (let i = 0; i < 17; i++) {
    for (let j = 0; j < 17; j++) {

      var temp = "highlightR - " + i + " - " + j;
...
function clearSelectedHighlight(scene, currentActor) {
  var color;
  if (currentActor.actor.name.includes("Enemy")) {
    color = "O";
  } else
    color = "P";

  var selectHighlightChar = scene.getObjectByName("highlight" + color + " - " + currentActor.actor.yPos + " - " + currentActor.actor.xPos);
  selectHighlightChar.visible = false;
}

function addSelectedHighlight(scene, currentActor) {
  var color;
  if (currentActor.actor.name.includes("Enemy")) {
    color = "O";
  } else
  ...
  var selectHighlightChar = scene.getObjectByName("highlight" + color + " - " + currentActor.actor.yPos + " - " + currentActor.actor.xPos);
  selectHighlightChar.visible = true;
}

export { createHighlights, moveRadius, characterRadius, clearCharRadius, clearEnemyRadius, clearSelectedHighlight, addSelectedHighlight };
